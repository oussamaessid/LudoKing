...
public class MainActivity extends AppCompatActivity {
    ...
    class Piece
    {
        ...
        PieceIndicatorView readyToPick;
        int startPosition;
        ...
        ImageView pieceIcon,pieceStandIcon,shadowCircle;
        boolean isThisPlayerWon = false;
        boolean isMoving = false;


        private final ObjectAnimator rotateAnimator;
        private ObjectAnimator diceAuraAnimator;
        private ObjectAnimator pulseAnimator;

        Piece(String color,ConstraintLayout piece,PieceIndicatorView readyToPick,ImageView pieceIcon,ImageView pieceStandIcon,ImageView shadowCircle,int startPosition,float defX,float defY,boolean isBotPiece)
        {
            this.colour = color;
            this.piece = piece;
            ...
            this.readyToPick.setVisibility(View.VISIBLE);
        ...
        void die()
        {
            ...
            hideDiceAura();
            shadowCircle.animate().cancel();
            shadowCircle.setScaleX(1.0f);
            shadowCircle.setScaleY(1.0f);
            checkAdjustments(currBlock);
            currWinnerBlock=0;
            numberOfSteps = 0;
            if(isSoundOn) {
        ...
        void activeState()
        {
            ...
            if (pulseAnimator == null) {
                pulseAnimator = ObjectAnimator.ofFloat(readyToPick, View.ALPHA, 0.35f, 1.0f);
                pulseAnimator.setDuration(480);
        ...
        void inactiveState()
        {
            ...
            readyToPick.setAlpha(1.0f);
            readyToPick.setDashed(false);
            readyToPick.setVisibility(View.VISIBLE);
            hideDiceAura();
            shadowCircle.setAlpha(0.55f);
            if(currBlock!=-1) {
                checkAdjustments(currBlock);
            }
        }

        void showDiceAura(int solidColor) {
            shadowCircle.animate().cancel();
            GradientDrawable aura = new GradientDrawable();
            aura.setShape(GradientDrawable.OVAL);
            aura.setColor((solidColor & 0x00FFFFFF) | 0x70000000);
            shadowCircle.setImageDrawable(aura);
            shadowCircle.setAlpha(1.0f);
            shadowCircle.setScaleX(0.8f);
            shadowCircle.setScaleY(0.8f);
            if (diceAuraAnimator != null) diceAuraAnimator.cancel();
            diceAuraAnimator = ObjectAnimator.ofPropertyValuesHolder(shadowCircle,
                    PropertyValuesHolder.ofFloat(View.SCALE_X, 0.8f, 1.2f),
                    PropertyValuesHolder.ofFloat(View.SCALE_Y, 0.8f, 1.2f));
            diceAuraAnimator.setDuration(620);
            diceAuraAnimator.setRepeatCount(ObjectAnimator.INFINITE);
            diceAuraAnimator.setRepeatMode(ObjectAnimator.REVERSE);
            diceAuraAnimator.setInterpolator(new LinearInterpolator());
            diceAuraAnimator.start();
        }

        void hideDiceAura() {
            if (diceAuraAnimator != null) {
                diceAuraAnimator.cancel();
                diceAuraAnimator = null;
            }
            shadowCircle.setScaleX(1.0f);
            shadowCircle.setScaleY(1.0f);
            shadowCircle.setImageDrawable(null);
        }
        ...
        void move(int n) {
            ...
            if (!isMoving) {
                isMoving = true;
                shadowCircle.animate().scaleX(1.45f).scaleY(1.45f).setDuration(160).start();
            }
            if(stepSound!=null) {
                if(isSoundOn) {
            ...
            float iconBase = pieceIcon.getScaleX();
            PropertyValuesHolder scaleX = PropertyValuesHolder.ofFloat(View.SCALE_X, iconBase, iconBase * 1.4f);
            PropertyValuesHolder scaleY = PropertyValuesHolder.ofFloat(View.SCALE_Y, iconBase, iconBase * 1.4f);
            ObjectAnimator popUpAnimator = ObjectAnimator.ofPropertyValuesHolder(pieceIcon, scaleX, scaleY);
            popUpAnimator.setDuration(100);
            scaleX = PropertyValuesHolder.ofFloat(View.SCALE_X, iconBase * 1.4f, iconBase);
            scaleY = PropertyValuesHolder.ofFloat(View.SCALE_Y, iconBase * 1.4f, iconBase);
            ObjectAnimator popDownAnimator = ObjectAnimator.ofPropertyValuesHolder(pieceIcon, scaleX, scaleY);
            ...
            globalHandler.postDelayed(() -> {
                if(n>1) {
                    ...
                    isMoving = false;
                    shadowCircle.animate().scaleX(1.0f).scaleY(1.0f).setDuration(220).start();
    ...
    private Piece getKillTarget(Piece mover, int diceValue) {
        if (!mover.isAlive || mover.isReadyToEnterWinnerZone) return null;
        int simulatedBlock = mover.currBlock;
        for (int step = 1; step <= diceValue; step++) {
            if (simulatedBlock >= 51) simulatedBlock = 0;
            else simulatedBlock++;
            if (simulatedBlock == mover.endPosition && step < diceValue) return null;
        }
        if (safeSpots.contains(simulatedBlock)) return null;
        int idx = currentPlayerIndex;
        if (idx > players.size() - 1) idx = players.size() - 1;
        for (int i = 0; i < players.size(); i++) {
            String colour = players.get(idx).getColor();
            if (!colour.equals(mover.colour)) {
                for (Piece ep : getPiecesByColor(colour)) {
                    if (ep.isAlive && ep.currBlock == simulatedBlock) {
                        if (gametype == 2) {
                            if ((mover.colour.equals("yellow") && ep.colour.equals("red")) ||
                                    (mover.colour.equals("red") && ep.colour.equals("yellow")) ||
                                    (mover.colour.equals("blue") && ep.colour.equals("green")) ||
                                    (mover.colour.equals("green") && ep.colour.equals("blue"))) continue;
                        }
                        return ep;
                    }
                }
            }
            if (idx >= players.size() - 1) idx = 0; else idx++;
        }
        return null;
    }
    ...
    private void showPenaltyMessage(String message) {
        if (penaltyMessageView == null) return;
        penaltyMessageView.setText(message);
        penaltyMessageView.setAlpha(0f);
        penaltyMessageView.setVisibility(View.VISIBLE);
        penaltyMessageView.animate().alpha(1f).setDuration(300).start();
        globalHandler.postDelayed(() -> penaltyMessageView.animate().alpha(0f).setDuration(500)
                .withEndAction(() -> penaltyMessageView.setVisibility(View.GONE)).start(), 3000);
    }
    ...
    class Dice
    {
        ...
        void roll() {
            ...
            diceHandler.postDelayed(() -> {
                ...
                if (ch == 6) {
                    int auraColor = getPieceColorInt(currentPlayerColor);
                    for (Piece p : pieces) {
                        if (!p.hasCompletedItsPurpose && !p.isThisPlayerWon) {
                            p.showDiceAura(auraColor);
                        }
                    }
                }
    ...
    private int getPieceColorInt(String colour) {
        switch (colour) {
            case "red":    return 0xFFEC1D27;
            case "green":  return 0xFF01EC6B;
            case "blue":   return 0xFF1293E5;
            case "yellow": return 0xFFE1D037;
            default:       return 0xFF1293E5;
        }
    }
    ...
    private Player getPlayerByColor(String color) {
        for (Player p : players) {
            if (p.color.equals(color)) return p;
        }
        return null;
    }
    ...
    private void createPieces(String color, int position,String playername,boolean isBot) {
        ...
        switch (color) {
            case "red":
                rp.add(new Piece("red", redpiece1, (PieceIndicatorView) findViewById(R.id.red1activecircle), findViewById(R.id.imageView6), findViewById(R.id.imageView7), findViewById(R.id.red1shadowcircle), sPos[position - 1], pos[0][0], pos[0][1],isBot));
                rp.add(new Piece("red", redpiece2, (PieceIndicatorView) findViewById(R.id.red2activecircle), findViewById(R.id.imageView62), findViewById(R.id.imageView72), findViewById(R.id.red2shadowcircle), sPos[position - 1], pos[1][0], pos[1][1],isBot));
                rp.add(new Piece("red", redpiece3, (PieceIndicatorView) findViewById(R.id.red3activecircle), findViewById(R.id.imageView63), findViewById(R.id.imageView73), findViewById(R.id.red3shadowcircle), sPos[position - 1], pos[2][0], pos[2][1],isBot));
                rp.add(new Piece("red", redpiece4, (PieceIndicatorView) findViewById(R.id.red4activecircle), findViewById(R.id.imageView64), findViewById(R.id.imageView74), findViewById(R.id.red4shadowcircle), sPos[position - 1], pos[3][0], pos[3][1],isBot));
                for (Piece p : rp) applyCellRing(p.shadowCircle);
                break;
            case "green":
                gp.add(new Piece("green", greenpiece1, (PieceIndicatorView) findViewById(R.id.green1activecircle), findViewById(R.id.imageView51), findViewById(R.id.imageView81), findViewById(R.id.green1shadowcircle), sPos[position - 1], pos[0][0], pos[0][1],isBot));
                gp.add(new Piece("green", greenpiece2, (PieceIndicatorView) findViewById(R.id.green2activecircle), findViewById(R.id.imageView52), findViewById(R.id.imageView82), findViewById(R.id.green2shadowcircle), sPos[position - 1], pos[1][0], pos[1][1],isBot));
                gp.add(new Piece("green", greenpiece3, (PieceIndicatorView) findViewById(R.id.green3activecircle), findViewById(R.id.imageView53), findViewById(R.id.imageView83), findViewById(R.id.green3shadowcircle), sPos[position - 1], pos[2][0], pos[2][1],isBot));
                gp.add(new Piece("green", greenpiece4, (PieceIndicatorView) findViewById(R.id.green4activecircle), findViewById(R.id.imageView54), findViewById(R.id.imageView84), findViewById(R.id.green4shadowcircle), sPos[position - 1], pos[3][0], pos[3][1],isBot));
                for (Piece p : gp) applyCellRing(p.shadowCircle);
                break;
            case "blue":
                bp.add(new Piece("blue", bluepiece1, (PieceIndicatorView) findViewById(R.id.blue1activecircle), findViewById(R.id.imageView41), findViewById(R.id.imageView91), findViewById(R.id.blue1shadowcircle), sPos[position - 1], pos[0][0], pos[0][1],isBot));
                bp.add(new Piece("blue", bluepiece2, (PieceIndicatorView) findViewById(R.id.blue2activecircle), findViewById(R.id.imageView42), findViewById(R.id.imageView92), findViewById(R.id.blue2shadowcircle), sPos[position - 1], pos[1][0], pos[1][1],isBot));
                bp.add(new Piece("blue", bluepiece3, (PieceIndicatorView) findViewById(R.id.blue3activecircle), findViewById(R.id.imageView43), findViewById(R.id.imageView93), findViewById(R.id.blue3shadowcircle), sPos[position - 1], pos[2][0], pos[2][1],isBot));
                bp.add(new Piece("blue", bluepiece4, (PieceIndicatorView) findViewById(R.id.blue4activecircle), findViewById(R.id.imageView44), findViewById(R.id.imageView94), findViewById(R.id.blue4shadowcircle), sPos[position - 1], pos[3][0], pos[3][1],isBot));
                for (Piece p : bp) applyCellRing(p.shadowCircle);
                break;
            case "yellow":
                yp.add(new Piece("yellow", yellowpiece1, (PieceIndicatorView) findViewById(R.id.yellow1activecircle), findViewById(R.id.imageView31), findViewById(R.id.imageView101), findViewById(R.id.yellow1shadowcircle), sPos[position - 1], pos[0][0], pos[0][1],isBot));
                yp.add(new Piece("yellow", yellowpiece2, (PieceIndicatorView) findViewById(R.id.yellow2activecircle), findViewById(R.id.imageView32), findViewById(R.id.imageView102), findViewById(R.id.yellow2shadowcircle), sPos[position - 1], pos[1][0], pos[1][1],isBot));
                yp.add(new Piece("yellow", yellowpiece3, (PieceIndicatorView) findViewById(R.id.yellow3activecircle), findViewById(R.id.imageView33), findViewById(R.id.imageView103), findViewById(R.id.yellow3shadowcircle), sPos[position - 1], pos[2][0], pos[2][1],isBot));
                yp.add(new Piece("yellow", yellowpiece4, (PieceIndicatorView) findViewById(R.id.yellow4activecircle), findViewById(R.id.imageView34), findViewById(R.id.imageView104), findViewById(R.id.yellow4shadowcircle), sPos[position - 1], pos[3][0], pos[3][1],isBot));
                for (Piece p : yp) applyCellRing(p.shadowCircle);
                break;
        }
    }

    private void applyCellRing(ImageView shadowCircle) {
        float density = displayMetrics.density;
        int   ringSize = (int)(pxWidth * 0.065);

        shadowCircle.setImageDrawable(null);

        ConstraintLayout.LayoutParams lp =
                (ConstraintLayout.LayoutParams) shadowCircle.getLayoutParams();
        lp.width  = ringSize;
        lp.height = ringSize;
        lp.topMargin = -(int)(6f * density) - ringSize / 2;
        shadowCircle.setLayoutParams(lp);
    }
