...
public class HomeActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    ConstraintLayout penaltyModeLayout;
    View penaltyMode1Card, penaltyMode2Card, penaltyMode3Card;
    TextView penaltyMode1SelectedTag, penaltyMode2SelectedTag, penaltyMode3SelectedTag;
    ...
    private void showPenaltyModeScreen(Intent intent) {
        ...
        updatePenaltyModeSelection();
        penaltyModeLayout.setScaleX(0.9f);
        penaltyModeLayout.setScaleY(0.9f);
        penaltyModeLayout.setAlpha(0f);
        penaltyModeLayout.setVisibility(View.VISIBLE);
        penaltyModeLayout.animate().scaleX(1f).scaleY(1f).alpha(1f)
                .setDuration(220).setInterpolator(new OvershootInterpolator(1.2f)).start();
    }

    private void updatePenaltyModeSelection() {
        penaltyMode1Card.setAlpha(selectedPenaltyMode == 1 ? 1f : 0.5f);
        penaltyMode2Card.setAlpha(selectedPenaltyMode == 2 ? 1f : 0.5f);
        penaltyMode3Card.setAlpha(selectedPenaltyMode == 3 ? 1f : 0.5f);
        penaltyMode1SelectedTag.setVisibility(selectedPenaltyMode == 1 ? View.VISIBLE : View.GONE);
        penaltyMode2SelectedTag.setVisibility(selectedPenaltyMode == 2 ? View.VISIBLE : View.GONE);
        penaltyMode3SelectedTag.setVisibility(selectedPenaltyMode == 3 ? View.VISIBLE : View.GONE);
    }

    private void initPenaltyModeLayout() {
        penaltyModeLayout = findViewById(R.id.penaltymodelayout);
        penaltyMode1Card = findViewById(R.id.penaltymode1card);
        penaltyMode2Card = findViewById(R.id.penaltymode2card);
        penaltyMode3Card = findViewById(R.id.penaltymode3card);
        penaltyMode1SelectedTag = findViewById(R.id.mode1selectedtag);
        penaltyMode2SelectedTag = findViewById(R.id.mode2selectedtag);
        ...
        penaltyMode2Card.setOnClickListener(v -> {
            selectedPenaltyMode = 2;
            updatePenaltyModeSelection();
        });
        penaltyMode3Card.setOnClickListener(v -> {
            selectedPenaltyMode = 3;
            updatePenaltyModeSelection();
        });

        penaltyModeBackBtn.setOnClickListener(v -> {
            penaltyModeLayout.animate().scaleX(0.9f).scaleY(0.9f).alpha(0f).setDuration(180)
                    .withEndAction(() -> penaltyModeLayout.setVisibility(View.GONE)).start();
        });
